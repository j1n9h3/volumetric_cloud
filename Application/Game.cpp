...
void Game::loadShaders(bool firstTimeLoadShaders)
{
	...
	success &= reload(&mCloudVertexShader, L"Resources\\Cloud.hlsl", "CloudVertexShader", firstTimeLoadShaders, nullptr, lazyCompilation);
	success &= reload(&mCloudPixelShader, L"Resources\\Cloud.hlsl", "CloudPixelShader", firstTimeLoadShaders, nullptr, lazyCompilation);
...
void Game::releaseShaders()
{
	...
	resetPtr(&mCloudVertexShader);
	resetPtr(&mCloudPixelShader);
...
void Game::render()
{
	//  Menu/imgui
	{
		...
		if (uiRenderingMethod == MethodPathTracing || uiRenderingMethod == MethodRaymarching)
		{
			...
			ImGui::Checkbox("Cloud", &RenderCloud);
		}
	...
	if (memcmp(&AtmosphereInfos, &AtmosphereInfosSaved, sizeof(AtmosphereInfo)) != 0 || uiRenderingMethodPrev != uiRenderingMethod
		...
		|| RenderTerrainPrev != RenderTerrain || RenderCloudPrev != RenderCloud)
	{
		forceGenLut |= true;
